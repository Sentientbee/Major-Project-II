...
function App() {
  ...
  const renderMessageContent = (content) => {
    ...
    while ((match = jsonRegex.exec(content)) !== null) {
      ...
      // 2. Parse and render the structured UI component using Recharts
      try {
        const data = JSON.parse(match[1]);
        const CHART_COLORS = [palette.chartBar, palette.primary, '#E6A89C', '#A8B8A6', '#8FA8A4', '#D4C5B9'];

        if (['bar_chart', 'line_chart', 'pie_chart'].includes(data.type)) {
          parts.push(
            <div key={match.index} style={{ margin: '16px 0', padding: '20px', background: '#fff', borderRadius: '12px', border: `1px solid ${palette.neutral}`, boxShadow: '0 2px 8px rgba(0,0,0,0.04)', height: '400px', width: '100%', boxSizing: 'border-box' }}>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  {data.type === 'bar_chart' ? (
                    <BarChart data={data.data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={palette.neutral} />
                      <XAxis dataKey="name" tick={{fill: palette.textDark}} axisLine={{stroke: palette.neutral}} />
                      <YAxis tick={{fill: palette.textDark}} axisLine={{stroke: palette.neutral}} />
                      <Tooltip contentStyle={{ borderRadius: '8px', border: `1px solid ${palette.neutral}` }} />
                      <Legend />
                      <Bar dataKey="value" fill={palette.chartBar} radius={[4, 4, 0, 0]} />
                    </BarChart>
                    ...
                    <LineChart data={data.data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <Tooltip contentStyle={{ borderRadius: '8px', border: `1px solid ${palette.neutral}` }} />
                      <Legend />
                      <Line type="monotone" dataKey="value" stroke={palette.chartBar} strokeWidth={3} activeDot={{ r: 8 }} />
                    </LineChart>
                  ) : (
                    <PieChart>
                      <Pie data={data.data} cx="50%" cy="50%" outerRadius={100} dataKey="value" label>
          ...
          // Fallback if AI generates unstructured JSON not matched to graphs
